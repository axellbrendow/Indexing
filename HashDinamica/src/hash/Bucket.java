...
public class Bucket<TIPO_DAS_CHAVES, TIPO_DOS_DADOS> implements Serializavel
{
    ...
    {
        this.bucket = bucket;
        this.profundidadeLocal = (profundidadeLocal < 1 ? PADRAO_PROFUNDIDADE_LOCAL
                : profundidadeLocal);
        this.numeroDeRegistrosPorBucket = (numeroDeRegistrosPorBucket < 1
                || numeroDeRegistrosPorBucket > Short.MAX_VALUE
    ...
    protected Registro<TIPO_DAS_CHAVES, TIPO_DOS_DADOS> obterRegistro(
            int indiceDoRegistro)
    {
        SerializavelHelper.lerBytes(registro, bucket,
                DESLOCAMENTO_CABECALHO
                        + indiceDoRegistro * registro.obterTamanhoMaximoEmBytes());
    ...
    protected int pesquisarEnderecoDaChaveEDoDado(TIPO_DAS_CHAVES chave,
            TIPO_DOS_DADOS dado)
    {
            ...
            resultado = (byte) percorrerRegistros(
                    (registro, deslocamento) ->
                    {
                        ...
                        if (registro.lapide == Registro.DESATIVADO)
                        {
                            ...
                            SerializavelHelper.escreverObjeto(registro, bucket,
                                    deslocamento);
            ...
            String delimitadorEntreOsCamposDoRegistro, boolean mostrarApenasAsChaves)
    {
        ...
        percorrerRegistros(
                ...
                {
                    toStr += "{" +
                            registro.toString(delimitadorEntreOsCamposDoRegistro,
        ...
        toStr = toStr.substring(0,
                toStr.length() - delimitadorEntreRegistros.length()) + " ]";
