...
public class Hash<TIPO_DAS_CHAVES, TIPO_DOS_DADOS>
{
    ...
    {
        return buckets.listarDadosComAChave(chave,
                diretorio.obterEnderecoDoBucket(chave));
    }
    ...
    private void inserirElementosDe(Bucket<TIPO_DAS_CHAVES, TIPO_DOS_DADOS> bucket)
    {
        ...
        {
            Registro<TIPO_DAS_CHAVES, TIPO_DOS_DADOS> registro = bucket
                    .obterRegistro(i);
            ...
            TIPO_DOS_DADOS dado)
    {
        ...
        if (numeroDeChamadas < 2)
        {
            ...
            Bucket<TIPO_DAS_CHAVES, TIPO_DOS_DADOS> bucketExcluido = buckets
                    .resetarBucket(enderecoDoBucket);

            long enderecoDoNovoBucket = buckets.criarBucket((byte) (resultado + 1));

            int antigoTamanhoDoDiretorio = (int) Math.pow(2, resultado);
            int tamanhoDoDiretorio = diretorio.obterTamanhoDoDiretorio();
            int indiceDoPonteiroParaOBucket = diretorio
                    .pesquisarPeloPonteiro(enderecoDoBucket);

            // atualiza alguns ponteiros no diretório para o novo bucket
            for (int i = indiceDoPonteiroParaOBucket
                    + antigoTamanhoDoDiretorio; i < tamanhoDoDiretorio; i += antigoTamanhoDoDiretorio)
            {
    ...
    {
        return diretorio != null && diretorio.fechar() &&
    ...
    {
        return (mostrarDiretorio ? ("Diretorio:\n" +
